  ...
  const storyContent: Record<string, React.ReactNode> = {
    ...
    diagnosis: (
      <div className="space-y-2 text-sm text-foreground">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {kb.symptoms.map((s) => (
            <span key={s} className="rounded-full border border-border/60 bg-background/40 px-2.5 py-0.5 text-xs text-muted-foreground">{s}</span>
          ))}
        </div>
      </div>
    ),
    ...
    outcome: (
      <div className="space-y-2 text-sm text-foreground">
        ...
        <p className="text-muted-foreground">Addressing this now prevents estimated losses of <strong>{formatCurrency(diagnosis.estimatedCost * 2.4)}</strong> from unplanned downtime.</p>
      </div>
    ),
  };
            ...
            return (
              <div key={step.id} className="flex gap-5">
                ...
                <div className="flex flex-col items-center">
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${step.border} ${step.bg}`}>
                    <Icon className={`h-4 w-4 ${step.color}`} />
                  </div>
                  {!isLast && <div className="my-1.5 w-px flex-1 bg-border/60" />}
                </div>
                {/* Content */}
      ...
      <div className="rounded-3xl border border-border bg-card p-6 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {([
            ...
            <div key={title} className="rounded-2xl border border-border/60 bg-background/40 p-4">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          Sources:&nbsp;
          {kb.sources.map((s) => (
            <span key={s.ref} className="rounded-full border border-border/60 bg-background/40 px-2 py-0.5">{s.title} — {s.ref}</span>
          ))}
        </div>
      </div>
    </div>
