...
import { KNOWLEDGE, apiAsk } from "@/lib/inference";
...
function AssistantPage() {
    ...
    () => [
      {
        ...
        sources: diagnosis ? article.sources.map((s) => `${s.title} — ${s.ref}`) : undefined,
      },
    ],
    [diagnosis, article],
  );
  ...
  const ask = async (text: string) => {
    ...
    setThinking(true);
    try {
      const { answer, failureType } = await apiAsk(text, diagnosis?.type);
      const sourceLabel = failureType ? `SafeFactory Knowledge Base — ${failureType}` : "SafeFactory Knowledge Base";
      setMessages((m) => [...m, { role: "assistant", text: answer, sources: [sourceLabel] }]);
    } catch {
      const t = text.toLowerCase();
      let body = article.bestPractices.join(" ");
      if (t.includes("repair") || t.includes("fix") || t.includes("procedure")) body = article.repair.join(" ");
      else if (t.includes("safe")) body = article.safety.join(" ");
      else if (t.includes("part") || t.includes("spare")) body = article.spares.join(", ");
      else if (t.includes("symptom")) body = article.symptoms.join(", ");
      else if (t.includes("prevent") || t.includes("recurrence")) body = article.preventive.join(" ");
      else if (t.includes("explain") || t.includes("failure"))
        body = `${article.cause} ${diagnosis ? `Detected pattern: ${diagnosis.fullName}. Root cause: ${diagnosis.rootCause}` : ""}`;
      setMessages((m) => [...m, { role: "assistant", text: body, sources: article.sources.map((s) => `${s.title} — ${s.ref}`) }]);
    } finally {
      setThinking(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        title={<>Your <span className="text-gradient">maintenance knowledge base</span></>}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="glass rounded-3xl p-6 lg:col-span-2">
          <div className="flex h-[520px] flex-col">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-2">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  {m.role === "assistant" && (
                    <div className="mr-2 mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-brand/12 ring-1 ring-brand/20">
                      <Bot className="h-4 w-4 text-brand" />
                    </div>
                  )}
                  <div
                    className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-relaxed backdrop-blur-sm ${
                      m.role === "user"
                        ? "bg-brand text-brand-foreground shadow-sm"
                        : "border border-white/30 bg-white/40 text-foreground"
                    }`}
                  >
                    <div>{m.text}</div>
                    {m.sources && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {m.sources.map((s) => (
                          <span key={s} className="rounded-full border border-white/30 bg-white/40 px-2 py-0.5 text-[10px] text-muted-foreground">
                            {s}
                          </span>
                        ))}
                      </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {SUGGESTED_PROMPTS.map((s) => (
                <button
                  key={s}
                  onClick={() => ask(s)}
                  className="rounded-full border border-white/30 bg-white/40 px-3 py-1 text-xs text-muted-foreground backdrop-blur-sm transition-all hover:border-brand/30 hover:bg-brand/8 hover:text-foreground hover:-translate-y-px"
                >
                  {s}
                </button>
              ))}
            ...
            {/* Input */}
            <form className="mt-3 flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); ask(q); }}>
              <Input
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder="Ask about symptoms, repair steps, safety or spare parts…"
                className="h-11 rounded-full border-white/30 bg-white/40"
              />
        ...
        <div className="glass rounded-3xl p-6">
          ...
          <div className="mt-6 rounded-xl border border-white/30 bg-white/30 p-3 text-xs text-muted-foreground backdrop-blur-sm">
  ...
  return (
    <div>
      ...
      <ul className="mt-1.5 space-y-1">
        {items.map((item, k) => (
          <li key={k} className="text-foreground/80">• {item}</li>
        ))}
      </ul>
    </div>
  );
